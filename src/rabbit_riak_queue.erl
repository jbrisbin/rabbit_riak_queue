...
publish(Msg, Props, S) ->
    rabbit_log:info("publish() ->~n MSG: ~p~n PROPS: ~p~n STATE: ~p", [Msg, Props, S]),
...
fetch(AckRequired, S = #s { name = QueueName }) ->
    ...
    {Ack, S1} =
		...
		Result = {Msg, true, Ack, Len},
		rabbit_log:info("fetch result: ~p", [Result]),
    {Result, S1}.
...
tx_commit(Txn, F, PropsF, S) ->
    rabbit_log:info("tx_commit(~p, ~p, ~p, ~p) ->", [Txn, F, PropsF, S]),
...
is_empty(#s { name = QueueName }) ->
    rabbit_log:info("is_empty(~p)", [QueueName]),
...
publish_state(Msg,
	...
	rabbit_log:info("publish_state() -> SEQ_ID: ~p", [SeqId]),
...
internal_ack3(F, SeqIds, S) ->
    {AllGuids, S1} =
        lists:foldl(
          ...
          {[], S},
          SeqIds),
		rabbit_log:info("internal_ack3() -> ~p~n STATE: ~p", [AllGuids, S1]),
